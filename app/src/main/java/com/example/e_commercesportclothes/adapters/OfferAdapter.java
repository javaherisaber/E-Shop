...
public class OfferAdapter extends RecyclerView.Adapter<OfferAdapter.ViewHolderOffer> {
    ...
    public OfferAdapter(Context context, List<Product> productList, OnAddProduct onAddProduct) {
    ...
    @NonNull
    @Override
    public ViewHolderOffer onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.list_best_price, parent, false);
        return new ViewHolderOffer(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolderOffer holder, int position) {
    ...
    class ViewHolderOffer extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public ViewHolderOffer(@NonNull View itemView) {
            super(itemView);
    ...
    private List<Favorite> addOrRemoveFavorite(Product product, boolean isAdd) {
        ...
        favorite.setRawPrice(product.getRawPrice());
        if (isAdd) {
            sportShopDatabase = SportShopDatabase.getInstance(context);
